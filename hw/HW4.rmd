...
```{r}
colnames(mono_socio) <- c("ParticipantID", "TypeOfQuestion", "QuestionCategory", "Response")
```
...
```{r}
n_participant <- mono_socio %>%
  distinct(ParticipantID) %>%
  nrow()

paste('1. Number of participants is', n_participant)
...
paste('2. Number of male of participants is', n_male)
paste('   Number of female of participants is', n_female)
```

```{r}
ed_levels <- mono_socio %>%
  filter(QuestionCategory == 'education') %>%
  distinct(Response) %>%
  deframe()

print('3. Education levels are:')
...
mono_socio %>%
  filter(QuestionCategory == 'education') %>%
  group_by(QuestionCategory, Response) %>%
  count()

# 4. Number of participants by education level:'
```
...
# 5. Number of participants by handedness:
```
...
Answer:
A histogram plot is usually used to visualize the distribution of a particular variable.

```{r}
library(ggplot2)

ggplot(mono, aes(RT)) + geom_histogram(bins = 100)
...
Answer:
I don't think that we observe normal distribution here. The mean is obviously skewed to the left. It is quite asymetrical. The right tail is considerably longer. There are a few suspicious outliers as demonstrated in the code cell below.

```{r}
paste('Min:', min(mono$RT), 'Max:', max(mono$RT))
```
...
```{r}
ggplot(mono, aes(RTlog)) + geom_density()
...
Answer:
This distribution looks a bit more like normal distribution, yet it's still not quite it. The left tail is longer than the right. The distribution is slightly skewed to the left. Some outliers are situated very far from the mean, outside the range of 3 sigmas.
...
```{r}
print('RTlog stats:')
paste('Standard deviaton is', sd(mono$RTlog))
paste('Min value is', min(mono$RTlog))
paste('Max value is', max(mono$RTlog))
paste('Mean value is', mean(mono$RTlog))
paste('Median value is', median(mono$RTlog))
...
left_boundary <- 600
right_boundary <- mean(mono$RT) + (sd(mono$RT) * 3)

left_boundary_log <- mean(mono$RTlog) - (sd(mono$RTlog) * 3)
right_boundary_log <- mean(mono$RTlog) + (sd(mono$RTlog) * 3)

mono1 <- 
  mono %>% 
  filter(RT > left_boundary & RT < right_boundary) %>%
  filter(RTlog > left_boundary_log & RTlog < right_boundary_log) %>%
  filter(TypeOfStimuli != 'FillerUnacceptable' & TypeOfStimuli != 'FillerAcceptable') %>%
...
ggline(mono1, x = "AcceptabilityJ", y = "RTlog", 
       add = "mean_se",
       color = "WordOrder", palette = "uchicago")
...
mean_rtlog_long <- 
  ...
  summarise(RTlog = round(mean(RTlog), digits = 2))
...
Answer:
P-value is greater than $\alpha$, thus we do not reject the null hypothesis.

What claim about logarithms of reaction time for Congruent and Incongruent stimuli can you make according to this test?

Answer:
Logarithms of reaction time are distributed in a similar way for both Congruent and Incongruent stimuli.
...
Answer:
P-value is greater than $\alpha$, so we do not reject the null hypothesis.

What claim about logarithms of reaction time for Congruent and Incongruent stimuli can you make now?

Asnwer:
The claim from 2.3 stands, we're more confident in it now since p-value is even bigger.

How can you interpret difference with the result of 2.3?

Answer:
A paired t-test simply calculates the difference between paired observations and then performs a 1-sample t-test on the differences. In other words, paired t-test and two-sample t-test are calculated differently. With paired t-test there's even less reason to reject the null hypothesis. The main difference is two-sample t-test is used for independent samples, while paired t-test is used when the sample are dependent in some way. In our current scenario the samples are evidently dependent, because the same participants are given different stimuli which they react to.


```{r}
sd(mean_rtlog$Congruent) / sqrt(140) + sd(mean_rtlog$Incongruent) / sqrt(140)
sd(mean_rtlog$diff) / sqrt(140)
```
...
As we can see, the answer is indeed the same.
...
Use appropriate statistical test to answer the following question: is there any statistically significant difference in logarithm of reaction time for different conditions (types of stimuli)? Choose the test and provide justification for your choice. Provide your code, results and interpretation. What is your answer to the question?
...
Answer:
Here, p-value is called `Pr(>F)`. The null hypothesis in our case is that there is no statistically significant difference in logarithm of reaction time for different types of stimuli. The alternative hypothesis is that there is a difference. P-value is less than 0.05 (which is also indicated by the significance code ‘*’). This means that we can reject the null hypothesis in favor of the alternative. In other words, we can say that there indeed is a statistically significant difference.
...
Answer:
Here we can look at `p adj` column and see that the only pair where there is statistically significant difference between logarithms of reaction time is SubjectiveComment-Material and Shape-Color, since it is the only row where the adjusted p-value is smaller that $\alpha$.